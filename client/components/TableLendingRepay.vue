<template>
    ...
    <section v-else>
        <p
            v-if="total > 0"
            class="wethio-total-items">{{ _nFormatNumber('lending repay', 'lending repays', total) }}</p>
        <form
            v-if="showFilter"
        ...
        <div
            ...
            <p class="wethio-empty__description">No repay found</p>
        </div>
        <table-base
            v-if="total > 0"
            :fields="fields"
            :items="items"
            class="wethio-table--lending-repay">
            <template
                slot="hash"
                slot-scope="props">
                <nuxt-link
