<template>
    ...
    <section v-else>
        ...
        <table-base
            ...
            :fields="fields"
            :items="items"
            class="wethio-table--holders">
            <template
                slot="hash"
                slot-scope="props">
                <nuxt-link
                    :to="{name: 'tokens-slug-zrc21-holder', params: { slug: address, holder: props.item.hash}}"
...
export default {
    ...
    methods: {
        async getDataFromApi () {
            ...
            const { data } = await this.$axios.get('/api/token-holders/zrc21' + '?' + query)
