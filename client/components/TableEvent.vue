<template>
    ...
    <section v-else>
        ...
        <div
            v-if="total == 0"
            class="wethio-empty">
            <i class="fa fa-binoculars wethio-empty__icon"/>
            <p class="wethio-empty__description">No event found</p>
        </div>

        <p
            v-if="total > 0"
            class="wethio-total-items">{{ _nFormatNumber('item', 'items', total) }}</p>

        <table-base
            v-if="total > 0"
            :fields="fields"
            :items="items"
            class="wethio-table--events">
