...
TokenTxController.get('/token-txs/:tokenType', [
    ...
    try {
        ...
        if (!config.get('GetDataFromElasticSearch')) {
            ...
            if (tokenType === 'zrc20') {
                eData = await elastic.search('zrc20-tx', query, { blockNumber: 'desc' }, limit, page)
                const count = await elastic.count('zrc20-tx', query)
                total = count.count
            } else if (tokenType === 'zrc21') {
                eData = await elastic.search('zrc21-tx', query, { blockNumber: 'desc' }, limit, page)
                const count = await elastic.count('zrc21-tx', query)
                total = count.count
            } else if (tokenType === 'trc721') {
